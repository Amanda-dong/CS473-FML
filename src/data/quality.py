...
def fill_feature_matrix_nulls(fm: pd.DataFrame, acs_df: pd.DataFrame | None = None) -> pd.DataFrame:
    ...
    with warnings.catch_warnings():
        ...
        # 3. NLP share cols
        share_cols = [
            'halal_related_share', 'explicit_halal_share', 'implicit_halal_share',
            'not_related_share', 'healthy_food_share', 'salad_bowls_share',
            'mediterranean_bowls_share', 'healthy_indian_share', 'smoothie_juice_share',
            'halal_fast_casual_share', 'subtype_gap'
        ]
        ...
        # 4. NLP rate cols
        rate_cols = [
            'overall_positive_rate', 'overall_negative_rate', 'halal_positive_rate',
            'halal_negative_rate', 'non_halal_positive_rate', 'non_halal_negative_rate',
        ...
        # 5. NLP count cols
        count_cols = [
            'total_review_count', 'unique_restaurant_count', 'halal_related_review_count',
            'explicit_halal_review_count', 'implicit_halal_review_count', 'not_related_review_count'
        ]
        ...
        if 'target' in df.columns:
            df['target'] = df.groupby('zone_id')['target'].transform(lambda x: x.fillna(x.mean()))
            df['target'] = df['target'].fillna(df['target'].median())

        # 9. Inspection grade avg static
        if 'inspection_grade_avg_static' in df.columns:
            df = df.sort_values(['zone_id', 'time_key'])
            df['inspection_grade_avg_static'] = df.groupby('zone_id')['inspection_grade_avg_static'].ffill().bfill()
            df['inspection_grade_avg_static'] = df['inspection_grade_avg_static'].fillna(df['inspection_grade_avg_static'].median())
        ...
        if acs_df is not None:
            ...
            acs_borough_medians = acs.groupby(['borough'])[['median_income', 'population', 'rent_burden']].median().reset_index()

            df = df.merge(acs_borough_medians, on=['borough'], how='left', suffixes=('', '_med'))
            ...
            df.drop(columns=['borough', 'median_income_med', 'population_med', 'rent_burden_med'], inplace=True)
        ...
        if 'rent_pressure' in df.columns:
            df['rent_pressure'] = df['rent_pressure'].fillna(df['rent_burden'] / df['rent_burden'].max())
