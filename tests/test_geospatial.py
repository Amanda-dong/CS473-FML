...
def test_load_nta_polygons_no_files_found(monkeypatch):
    monkeypatch.setattr("src.utils.geospatial._NTA_GEOJSON_CANDIDATES", ["nonexistent.geojson"])
    codes, tree = _load_nta_polygons()
    assert codes == []
    assert tree is None
...
def test_load_nta_polygons_skips_empty_nta2020(monkeypatch, tmp_path):
    ...
    data = {
        "features": [
            {"properties": {"nta2020": ""}, "geometry": {"type": "Point", "coordinates": [0, 0]}},
            {"properties": {"nta2020": "NTA1"}, "geometry": {"type": "Point", "coordinates": [0, 0]}}
        ]
    }
...
def test_load_nta_polygons_handles_shape_exception(monkeypatch, tmp_path):
    ...
    data = {
        "features": [
            {"properties": {"nta2020": "BAD"}, "geometry": {"type": "Point", "coordinates": "invalid"}},
        ]
    }
    f.write_text(json.dumps(data))
    monkeypatch.setattr("src.utils.geospatial._NTA_GEOJSON_CANDIDATES", [str(f)])
...
def test_lat_lon_to_nta_centroid_fallback_exception(monkeypatch):
    ...
    monkeypatch.setattr("src.utils.geospatial._load_nta_polygons", lambda: (["NTA1"], mock_tree))
