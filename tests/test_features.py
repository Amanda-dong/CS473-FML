...
def test_build_zone_year_matrix_acs_overlap_drop_coverage(monkeypatch) -> None:
    ...
    monkeypatch.setattr(lv, "build_license_velocity_features", lambda df: pd.DataFrame({
        "zone_id": ["BK0202"],
        "time_key": [2024],
...
def test_build_zone_year_matrix_gemini_overlap_drop_coverage(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr(ra, "aggregate_healthy_review_features", 
                       lambda df: pd.DataFrame({"zone_id": ["bk-downtownbk"], "time_key": [2024], "halal_related_share": [0.8]}))
    ...
    result = build_zone_year_matrix(etl_outputs)
    assert "halal_related_share" in result.columns
    val = result.loc[result["zone_id"] == "bk-downtownbk", "halal_related_share"].iloc[0]
    assert val == 0.8
