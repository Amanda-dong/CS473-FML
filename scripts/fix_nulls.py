...
def audit_and_fix():
    ...
    df_census["median_household_income"] = df_census["median_household_income"].fillna(
        0
    )
    ...
    print(
        f"yelp_business_zones.csv: NTA nulls {before_yelp_nta} -> {after_yelp_nta}, Zone nulls {before_yelp_zone} -> {after_yelp_zone}"
    )

    # 3 & 4. gemini_labels_full.csv and gemini_labels_halal_filtered.csv
    label_files = [
        "data/raw/gemini_labels_full.csv",
        "data/raw/gemini_labels_halal_filtered.csv",
    ]
    ...
    for f in label_files:
        ...
        df["time_key"] = (
            df["time_key"].fillna(df["review_date"].dt.year).fillna(2023).astype(int)
        )
        ...
        print(
            f"{f}: Time nulls {before_time} -> {after_time}, Zone nulls {before_zone} -> {after_zone}"
        )
