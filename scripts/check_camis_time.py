...
def main() -> None:
    ...
    print("Years available:", sorted(df["INSPECTION DATE"].dt.year.dropna().unique().tolist()))
    ...
    insp["is_grade_a"] = insp["grade"].fillna("").astype(str).str.upper().eq("A").astype(int)

    agg = (
        insp.groupby(["nta_id", "year"], as_index=False)
        .agg(grade_a_rate=("is_grade_a", "mean"), n_inspections=("restaurant_id", "count"))
    )

    g2022 = agg[agg["year"] == 2022].rename(
        columns={"grade_a_rate": "grade_a_rate_2022", "n_inspections": "n_inspections_2022"}
    )[["nta_id", "grade_a_rate_2022", "n_inspections_2022"]]
    g2024 = agg[agg["year"] == 2024].rename(
        columns={"grade_a_rate": "grade_a_rate_2024", "n_inspections": "n_inspections_2024"}
    )[["nta_id", "grade_a_rate_2024", "n_inspections_2024"]]
    both = g2022.merge(g2024, on="nta_id", how="inner")
    both_valid = both[(both["n_inspections_2022"] >= 5) & (both["n_inspections_2024"] >= 5)].copy()
    ...
    join_key = next((c for c in ["review_id", "restaurant_id", "business_id"] if c in reviews.columns and c in gemini.columns), None)
    label_col = next((c for c in ["halal_label", "label", "gemini_label", "category", "halal_relevance"] if c in gemini.columns), None)
    ...
    joined["is_halal"] = (
        joined[label_col].fillna("").astype(str).str.contains("halal", case=False, regex=False).astype(int)
    )
    ...
    y2022 = agg[(agg["year"] == 2022) & (agg["total_reviews"] >= 5)].rename(
        columns={"total_reviews": "total_reviews_2022", "halal_related_share": "halal_related_share_2022"}
    )[["nta", "total_reviews_2022", "halal_related_share_2022"]]
    y2023 = agg[(agg["year"] == 2023) & (agg["total_reviews"] >= 5)].rename(
        columns={"total_reviews": "total_reviews_2023", "halal_related_share": "halal_related_share_2023"}
    ...
    halal_new = halal_new[halal_new["INSPECTION DATE"].dt.year.between(2010, 2025)].copy()
    ...
    camis_nta = (
        halal_new.sort_values("INSPECTION DATE")
    ...
    ntas_all_three = sorted([nta for nta, years in nta_year_sets.items() if needed_years.issubset(years)])
